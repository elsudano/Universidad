...
def _config_allauth_in_mongodb():
    run("mongomongodb://%(mongodb_host)s:%(mongodb_port)s/%(name_dbapp)s --eval \
...
def _import_data_mongodb():
    run("mongomongodb://%(mongodb_host)s:%(mongodb_port)s/%(name_dbapp)s --eval \"db.%(name_app)s_restaurants.drop()\"" % env)
    run("mongomongodb://%(mongodb_host)s:%(mongodb_port)s/%(name_dbapp)s --eval \"db.%(name_app)s_neighborhoods.drop()\"" % env)
