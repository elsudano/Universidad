...
// mirar la direccion 0
    ...
    P1=P2=P3 = false;
    if (C0==0 and C1>=0 and C2>=0 and C3>=0 and Direccion==0){
        P1 = true;
    }
    if (C0==0 and C1>=0 and C2>=0 and C3>=0 and Direccion==1){
        P1 = true;
    }
    if (C0==0 and C1>=0 and C2>=0 and C3>=0 and Direccion==2){
        P1 = true;
    }
    if (C0==0 and C1>=0 and C2>=0 and C3>=0 and Direccion==3){
        P1 = true;
    }
//----------------------------------------------------------------
    if (C0==0 and C1==0 and C2>=0 and C3>=0 and Direccion==0){
...
//----------------------------------------------------------------
    ...
    if (dirty_){
        ...
        ActualizarMapa(PosX,PosY,false); // Actualizo la posición DONDE ESTOY como visitada
        if (P3 and !P4){
            cout << "Regla P3 no P4" << endl;
            accion = actTURN_R;
            Direccion = mod(Direccion+1,4);
        }else if (P2 and !P3 and !P4){
            cout << "Regla P2 no P3 no P4" << endl;
            accion = actTURN_L;
            Direccion = mod(Direccion-1,4);
        }else if (P1 and !P2 and !P3 and !P4){
            if (C1>C3){
                cout << "Regla P1 no P2 no P3 no P4 con if" << endl;
                accion = actTURN_L;
                Direccion = mod(Direccion-1,4);
            }else{
                cout << "Regla P1 no P2 no P3 no P4 con else" << endl;
                accion = actTURN_R;
                Direccion = mod(Direccion+1,4);
            }
        }
        else{
            cout << "Regla Final" << endl;
            accion = actFORWARD;
        }
...
void Agent::ImprimirMapa(char* val){
    if (val=="P"){
        ...
        cout << "P01: " << P1 << " P02: " << P2 << " P03: " << P3 << " P04: " << P4 << endl;//" P05: " << P5 << " P06: " << P6 << " P07: " << P7 << " P08: " << P8 << endl;
//        cout << "P09: " << P9 << " P10: " << P10 << " P11: " << P11 << " P12: " << P12 << " P13: " << P13 << " P14: " << P14 << " P15: " << P15 << " P16: " << P16 << endl;
        cout << "------------------------------------------------------------" << endl;
        cout << endl;
    }else if("F"){
        for (int Y=0; Y < tamTablero; Y++){
            for (int X=0; X < tamTablero; X++){
                cout << mapa[X+(Y*tamTablero)] << " ";
            }
            cout << endl;
        }
        cout << endl;
...
//------------------------------------------------------------
void Agent::ActualizarMapa(int X, int Y, bool muro){
    if (muro)
