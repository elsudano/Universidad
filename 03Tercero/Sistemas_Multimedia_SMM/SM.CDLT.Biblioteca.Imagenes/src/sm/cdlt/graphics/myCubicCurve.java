 ...
 */
public class myCubicCurve extends myCurveShape {
     ...
     * Constante que indica cual es el punto final de la curva.
     */
    private final int SECOND_CONTROL_POINT = 3;
    /**
     ...
     */
    public myCubicCurve(geoPoint p) {
        super(p, p);
     ...
     */
    public myCubicCurve(geoPoint pb, geoPoint pe) {
        super(pb, pe);
     ...
     */
    public myCubicCurve(geoPoint pb, geoPoint pe, Color c, float s) {
        super(pb, pe, c, s);
        this.list_of_points.add(pb);
        this.list_of_points.add(pe);
        this.status = this.FIRST_CONTROL_POINT;
    ...
    public void updateCurve(geoPoint p) {
        ...
        } else if (this.status == this.SECOND_CONTROL_POINT) {
            this.list_of_points.set(this.SECOND_CONTROL_POINT, p);
        } else {
    ...
    public void setLocation(geoPoint p) {
        ...
        pfijox = p.getPosX() - ((CubicCurve2D) this.myForma).getBounds().getX();
        pfijoy = p.getPosY() - ((CubicCurve2D) this.myForma).getBounds().getY();
    ...
    public void drawShapeIn(Graphics2D g2d) {
        ...
        ctrlx2 = this.list_of_points.get(this.SECOND_CONTROL_POINT).getPosX();
        ctrly2 = this.list_of_points.get(this.SECOND_CONTROL_POINT).getPosY();
        x2 = this.list_of_points.get(this.END_POINT).getPosX();
        y2 = this.list_of_points.get(this.END_POINT).getPosY();
        this.myForma = new CubicCurve2D.Double(x1, y1, ctrlx1, ctrly1, ctrlx2, ctrly2, x2, y2);
