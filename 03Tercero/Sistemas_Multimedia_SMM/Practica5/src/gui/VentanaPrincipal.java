...
public class VentanaPrincipal extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        JPanelLienzo = new practica5.Lienzo();
        ...
        jToggleButtonPintar.setIcon(new javax.swing.ImageIcon(getClass().getResource("/resources/pintar.png"))); // NOI18N
    ...
    private void jMenuItemNuevoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jMenuItemNuevoActionPerformed
        jToggleButtonPunto.setSelected(true);
        jToggleButtonPintar.setSelected(true);
        jCheckBoxRelleno.setSelected(false);
        jSliderGrosor.setValue(4);
        JPanelLienzo.clean();
        repaint();
    ...
    private void JPanelLienzoMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_JPanelLienzoMousePressed
        Shape aux = null;
        if (this.jToggleButtonPintar.isSelected()) {
            if (this.jToggleButtonPunto.isSelected()) {
                this.JPanelLienzo.setFill(true);
                aux = new Ellipse2D.Float(evt.getX() - this.jSliderGrosor.getValue() / 2, evt.getY() - this.jSliderGrosor.getValue() / 2, this.jSliderGrosor.getValue(), this.jSliderGrosor.getValue());
            } else if (this.jToggleButtonLinea.isSelected()) {
                this.JPanelLienzo.setFill(this.jCheckBoxRelleno.isSelected());
                aux = new Line2D.Float(evt.getPoint(), evt.getPoint());
            } else if (this.jToggleButtonRectangulo.isSelected()) {
                this.JPanelLienzo.setFill(this.jCheckBoxRelleno.isSelected());
                aux = new Rectangle2D.Float(evt.getX(), evt.getY(), 0, 0);
            } else if (this.jToggleButtonCirculo.isSelected()) {
                this.JPanelLienzo.setFill(this.jCheckBoxRelleno.isSelected());
                aux = new Ellipse2D.Float(evt.getX(), evt.getY(), 0, 0);
            }
        }
        this.JPanelLienzo.setSize(this.jSliderGrosor.getValue());
        if (aux != null) {
            this.JPanelLienzo.setShape(aux);
        }
        repaint();
    ...
    private void JPanelLienzoMouseDragged(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_JPanelLienzoMouseDragged
        Shape aux;
        if (this.jToggleButtonPintar.isSelected()) {
            aux = this.JPanelLienzo.getShape();
            if (this.jToggleButtonPunto.isSelected()) {
                ;// como esta es una herramienta particular lo dejamos así
            } else if (aux instanceof Line2D.Float) {
                Line2D.Float line_aux = (Line2D.Float) aux;
                line_aux.setLine(line_aux.getP1(), evt.getPoint());
            } else if (aux instanceof Rectangle2D.Float) {
                Rectangle2D.Float rect_aux = (Rectangle2D.Float) aux;
                rect_aux.setRect(rect_aux.x, rect_aux.y, evt.getX() - rect_aux.x, evt.getY() - rect_aux.y);
            } else if (aux instanceof Ellipse2D.Float) {
                Ellipse2D.Float elli_aux = (Ellipse2D.Float) aux;
                elli_aux.setFrame(elli_aux.x, elli_aux.y, evt.getX() - elli_aux.x, evt.getY() - elli_aux.y);
            }
        } else {
            aux = this.JPanelLienzo.getSelectedShape(evt.getPoint());
            // se puede implementar pero hay que hacer una clase propia
            // que extienda de Line2D para mover las lineas
            if (aux instanceof Rectangle2D.Float) {
                Rectangle2D.Float rect_aux = (Rectangle2D.Float) aux;
                rect_aux.setRect(evt.getX()-rect_aux.width/2, evt.getY()-rect_aux.height/2, rect_aux.width, rect_aux.height);
            } else if (aux instanceof Ellipse2D.Float) {
                Ellipse2D.Float elli_aux = (Ellipse2D.Float) aux;
                elli_aux.setFrame(evt.getX()-elli_aux.width/2, evt.getY()-elli_aux.height/2, elli_aux.width, elli_aux.height);
            }
        }
    ...
    private practica5.Lienzo JPanelLienzo;
