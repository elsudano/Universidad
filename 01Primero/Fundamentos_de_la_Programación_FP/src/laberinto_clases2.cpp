...
class laberinto{
    private:
        int filas, columnas;
        vector<vector<char>> lab, resuelto;
...
void laberinto::pintar(const vector<vector<bool> > &vec) const{
    for (int c=0;c<vec.size();c++){
        cout << " ";
        for (int c2=0;c2<vec.at(c).size();c2++)
            if (vec.at(c).at(c2)==false)
                cout << "0";
            else if (vec.at(c).at(c2)==true)
                cout << "1";
        cout << endl;
    }
}
...
// Funcion para pintar todo el contenido de un vector.
void laberinto::pintar_datos() const{
    for (int c=0;c<(*this).lab.size();c++){
        for (int c2=0;c2<(*this).lab.at(c).size();c2++)
            cout << (*this).lab.at(c).at(c2) << " ";
    }
