...
import gui.VentanaInternaBlend;
...
public class QuestionsSelectImages extends Questions {
    ...
    /**
     * Variable con una lista de Items que dentro tiene el nombre de la ventana
     * y la posición en la que se encuentra.
     */
    private String[] new_items;
    /**
     * Lista de imagenes que se van a mezclar
     */
     ...
     * ventana principal
     */
    private VentanaInterna[] vi_lista;
    ...
    private void myInitComponent() {
        ...
        this.vi_lista = new VentanaInterna[aux_list.length];
        this.new_items = new String[aux_list.length];
        String title;
        for (int i = 0; i < aux_list.length; i++) {
            title = aux_list[i].getTitle();
            this.vi_lista[i] = (VentanaInterna) aux_list[i];
            if (title != null && !title.equals("")) {
                this.new_items[i] = title;
            }
        }
        this.model_list.setCadenas(this.new_items);
    ...
    private void jListWindowsValueChanged(javax.swing.event.ListSelectionEvent evt) {//GEN-FIRST:event_jListWindowsValueChanged
        ...
        for (String val : this.jListWindows.getSelectedValuesList()) {
            for (VentanaInterna vi : this.vi_lista) {
    ...
    habiendo seleccionado las imagenes.
     */
    /**
     * Con este método podemos decirle a la ventana cual será la operación que
     * llevara a cabo despúes de seleccionar las diferentes imagenes.
     *
