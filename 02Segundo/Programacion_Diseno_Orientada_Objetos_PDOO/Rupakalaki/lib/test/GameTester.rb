  ...
  class GameTester
    ...
    def play(aGame, numberOfPlayers)
      ...
      begin #Mientras dure la partida
        ...
        begin #Mientras el jugador se decide a conocer al monstruo
          ...
          command = processCommand(command, current_player)        
        ...
        if (command == Command::ShowMonster) then
          ...
          if (command == Command::Combat) then
            combat_result = @game.developCombat()
            case combat_result
            ...
            if (combat_result != NapakalakiGame::CombatResult::WINGAME) then
              begin #Hasta que se avance de turno 
                puts "******* ******* ******* ******* ******* ******* *******"
                puts "\n\n Turno de: " + current_player.to_s()
                if current_player.canISteal then
    ...
    def getTreasure (howMany) 
      ...
      begin #tratar la excepcion
        ...
        valid_input = false if (option < Command::GoBack.menu || option > howMany)
      rescue Exception => e  
        valid_input = false
      end
    ...
    def manageDiscardTreasures (visible, aPlayer)
      ...
      begin #Se descartan tesoros hasta que se vuelve al men̼ anterior
        if visible then
          how_many = showTreasures("Elige tesoros visibles para descartar", aPlayer.getVisibleTreasures(), true)
        else 
          how_many = showTreasures("Elige tesoros ocultos para descartar", aPlayer.getHiddenTreasures(), true)
        end
    ...
    def manageMakeTreasureVisible (aPlayer)
      ...
      begin #Se hacen tesoros visibles hasta que se vuelve al men̼ anterior
        how_many = showTreasures("Elige tesoros para intentar hacerlos visibles", aPlayer.getHiddenTreasures(), true)
        option = getTreasure (how_many);
        if (option > -1) then
    ...
    def processCommand (command, aPlayer) 
      ...
      when Command::StealTreasure then
        a_treasure = aPlayer.stealTreasure;
        if a_treasure == nil
